...
class DmlTS(BaseTS):
    ...
    def update(self, context: np.ndarray, context_extra: np.ndarray, user_idx: np.ndarray, time_idx: np.ndarray, action: np.ndarray, reward: np.ndarray) -> 'DmlTS':
        # Assumption: users appear no more than once
        n_obs = context.shape[0]
